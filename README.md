...
**eva.rb** -- A utility and demo script which exercises the full capabilities of `lib/ppstrnum`'s `thousands`, `numbernames`, `pp_numstack`, `asc_numstack` and `desc_numstack` methods. Accepts one or more command-line arguments (`""`-quoted) as an arithmetical expressions to `eval`.
